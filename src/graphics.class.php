...
class Graphics
{
    ...
    public static function getRGBAFromColor($color)
    {
        if (is_array($color)) {
            ...
            return [$color >> 16, ($color >> 8) & 0x00FF, $color & 0x0000FF, 0];
        } elseif (preg_match('/^#?([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i', $color, $regs)) {
            return [hexdec($regs[1]), hexdec($regs[2]), hexdec($regs[3]), 0];
            ...
            return [hexdec($regs[1] . $regs[1]), hexdec($regs[2] . $regs[2]), hexdec($regs[3] . $regs[3]), 0];
        } elseif (preg_match('/^#?([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])$/i', $color, $regs)) {
            return [hexdec($regs[1] . $regs[1]), hexdec($regs[2] . $regs[2]), hexdec($regs[3] . $regs[3]), hexdec($regs[4] . $regs[4])];
        }
        return [0, 0, 0, 0];
    }
