...
abstract class SOME extends \ArrayObject
{
   ...
   /**
    ...
    public function __isset($var)
    {
        $key = self::typeof($var);
        switch ($key) {
            case self::FIELD_REFERENCE: case self::FIELD_LINK: case self::FIELD_COGNIZABLE: case self::FIELD_META:
                return isset($this->{self::$varsByTypes[$key]}[$var]);
                break;
            case self::FIELD_CHILDREN: case self::FIELD_PARENTS:
    ...
    final public function children($ref = null, $where = '', $addSQL = '', $max_level = 1)
    {
        ...
        do {
            ...
            $ch = array_map(function($x) use ($classname) { return new $classname($x); }, $ch);
            $children = array_merge($children, $ch);
            $ch = array_map(function($x) { return $x->_id; }, $ch);
    ...
    public static function getSet()
    {
        ...
        $sql['where'] = array_map(function($x) { return "(" . $x . ")"; }, array_filter($sql['where'], 'trim'));
    ...
    {
        $temp = array_filter(static::$references, function($x) use ($FK) { return $x['FK'] == $FK; });
        if ($temp) {
        ...
        foreach ($row as $key => $val) {
            ...
            switch(self::typeof($key)) {
                case self::FIELD_ID:
                    ...
                    break;
                case self::FIELD_REGULAR: case self::FIELD_META: case self::FIELD_COGNIZABLE:
    ...
    final private function __get_links($var)
    {
        ...
        return ($ids ? array_map(function($x) { return $x->_id; }, $this->linked[$var]) : $this->linked[$var]);
    }
    ...
    final private function __get_links_getLinked($var, $classname = null)
    {
        ...
        if (static::$links[$var]['field_to']) {
            if ($classname) {
                ...
                $this->linked[$var] = array_map(function($x) use ($classname) { return new $classname($x); }, (array)$SQL_result);
    ...
    final private function __get_children($var)
    {
        ...
        if (!isset($this->_children[$var2])) {
            ...
            if ($all) {
                $this->_children[$ref] = array_filter($this->_children[$var2], function($x) { return $x->__level == 1; });
            }
        }
        return ($ids ? array_map(function($x) { return $x->_id; }, $this->_children[$var2]) : $this->_children[$var2]);
    }
    ...
    final private function __get_parents($var)
    {
        ...
        return ($ids ? array_map(function($x) { return $x->_id; }, $this->_parents[$ref]) : $this->_parents[$ref]);
    }
    ...
    final private static function onupdate(array $ids, $ondelete = false)
    {
        ...
        $SQL = static::$SQL;
        $ids = array_map(function($x) use ($SQL) { return $SQL->quote($x); }, $ids);
        ...
        foreach ($classes as $classname) {
            ...
            if ($classname::$objectCascadeUpdate) {
                ...
                foreach ($SQL_result as $row) {
                    $row->trust();
                    $affected = array_filter(array_keys($row->meta), function($x) { return substr($x, 0, 8) == '__SOME__'; });
    ...
    final private static function ondelete_cascadeDelete($SQL_query)
    {
        ...
        if ($SQL_result) {
            if (static::$objectCascadeDelete) {
                ...
                $idN = static::_idN();
                $SQL = static::$SQL;
                $new_ids = array_map(function($x) use ($idN) { return $x[$idN]; }, $SQL_result);
                $sql_new_ids = array_map(function($x) use ($SQL) { return $SQL->quote($x); }, $new_ids);
